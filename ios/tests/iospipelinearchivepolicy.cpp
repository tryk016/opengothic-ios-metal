...
int main() {
  ...
  static_assert(Archive::TestModeTemporaryFileName==
    "RendererIOS-abi-10.binaryarchive.test-mode.tmp");
  static_assert(
  ...
  assert(provenance.find("metallib-abi=10\n")!=std::string::npos);
  ...
  assert(!Archive::provenanceMatches(
    mutateField("metallib-abi=10","metallib-abi=8"),digest));
  assert(!Archive::provenanceMatches(
    mutateField(
      "archive-file=RendererIOS-abi-10.binaryarchive",
      "archive-file=RendererIOS-abi-8.binaryarchive"),digest));
