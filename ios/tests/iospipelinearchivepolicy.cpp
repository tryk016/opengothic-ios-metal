...
int main() {
  ...
  static_assert(Archive::TestModeTemporaryFileName==
    "RendererIOS-abi-11.binaryarchive.test-mode.tmp");
  static_assert(
  ...
  assert(provenance.find("metallib-abi=11\n")!=std::string::npos);
  ...
  assert(!Archive::provenanceMatches(
    mutateField("metallib-abi=11","metallib-abi=8"),digest));
  assert(!Archive::provenanceMatches(
    mutateField(
      "archive-file=RendererIOS-abi-11.binaryarchive",
