...
readonly SHADING_PROTOTYPE_TILE_SELF_TEST_ARMED='RendererIOS shading prototype tile self-test: ARMED case=tile-prototype-v1 contract=1 metallib-abi=10 minimum-apple=4 output=4x4 rgba8-private=1'
...
rg -F 'RendererIOS shader library: source=offline-metallib resource=RendererIOS.metallib abi=10' \
  "$WORK/log.txt" >/dev/null || fail "offline metallib marker is missing"
rg -F 'RendererIOS builtin shader library: source=offline-metallib resource=RendererIOS.metallib abi=10 manifest=1 fail-closed=1' \
  "$WORK/log.txt" >/dev/null || fail "offline Builtin manifest marker is missing"
rg -F 'RendererIOS inventory shader manifest: resource=RendererIOS.metallib abi=10 manifest=1 exact-spirv=1 configured=1 fail-closed=1' \
  "$WORK/log.txt" >/dev/null || fail "offline inventory manifest marker is missing"
rg -F 'RendererIOS inventory shader pipeline: source=offline-metallib resource=RendererIOS.metallib abi=10 manifest=1 exact-spirv=1 functions-resolved=2 pipeline-wrapper-created=1' \
  "$WORK/log.txt" >/dev/null || fail "offline inventory pipeline marker is missing"
rg -F 'RendererIOS native Bink pipeline: source=offline-metallib resource=RendererIOS.metallib abi=10 color=rgba8 sample-count=1 pipeline-created=1' \
